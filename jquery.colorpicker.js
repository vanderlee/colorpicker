...
(function ($) {
	...
	var _colorpicker_index = 0,
        ...
        Color = function () {
			...
			this.distance = function(rgb) {
				var model = this.getRGB();
				return Math.pow(Math.abs(rgb.r - model.r), 2)
					 + Math.pow(Math.abs(rgb.g - model.g), 2)
					 + Math.pow(Math.abs(rgb.b - model.b), 2);
			}
